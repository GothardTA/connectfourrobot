...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Currently running on a {}", DeviceInfo::new()?.model());
    
    let mut gpio23 = Gpio::new()?.get(23)?.into_output();
    let mut gpio24 = Gpio::new()?.get(24)?.into_output();
    let mut gpio25 = Gpio::new()?.get(25)?.into_output();
    let mut gpio12 = Gpio::new()?.get(12)?.into_input_pullup();
    println!("Initialized gpio pins");
    println!("Waiting for button input...");
    ...
    loop {
        if gpio12.is_low() {
            ...
            // let img = ImageReader::open("/home/pi/image.jpg").expect("Failed to open file").decode().expect("Failed to decode image").into_rgba8();
            println!("SUccesfully opened image!");
